...
const RUST_BINARIES: RustBinaryName[] = ['agentpay-daemon', 'agentpay-admin', 'agentpay-agent'];
...
function buildWarnings(result: Omit<WalletStatusResult, 'security'>): string[] {
  ...
  if (result.config.readable && result.agent.legacyConfigToken.present) {
    if (result.agent.legacyConfigToken.keychainMatch === true) {
      ...
      warnings.push(
        `legacy agentAuthToken is still present in config.json; migrate it to ${storageLabel}`,
      );
    }
  }
  ...
  if (result.agent.agentKeyIdValid && result.agent.keychain.error) {
    ...
    warnings.push(
      `${storageLabel} does not contain an agent auth token for the configured agentKeyId`,
    );
  }
...
export function getWalletStatus(deps: WalletStatusDeps = {}): WalletStatusResult {
  ...
  const hasAgentAuthToken =
    deps.hasAgentAuthToken ??
    ((agentKeyId: string) => hasStoredAgentAuthToken(agentKeyId, platform));
