...
function findNearestPackageRoot(startPath: string): string | null {
  const current = path.resolve(startPath);
...
function resolveCurrentGlobalNpmCliInstallPath(): string | null {
  ...
  return path.resolve(packageRoot) === path.resolve(expectedInstallPath)
    ? expectedInstallPath
    : null;
}
...
function uninstallCurrentGlobalNpmCli(): GlobalCliUninstallResult {
  ...
  const uninstallResult = spawnSync('npm', ['uninstall', '--global', GLOBAL_NPM_CLI_PACKAGE_NAME], {
    encoding: 'utf8',
  });
...
export function cleanupLocalAdminResetState(
  ...
  let configValue: Record<string, unknown> | null = configExists
    ? redactConfig(currentConfig ?? {})
    : null;
  ...
  try {
    ...
    return {
      manifest: {
        ...
        agentpayHome:
          typeof parsed.agentpayHome === 'string' ? parsed.agentpayHome.trim() : undefined,
        shellRcPath: typeof parsed.shellRcPath === 'string' ? parsed.shellRcPath.trim() : undefined,
...
function removeManagedPathShim(
  targetPath: string,
  deps: Pick<CleanupLocalAdminUninstallStateDeps, 'existsSync' | 'readFileSync' | 'rmSync'>,
): boolean {
  const existsSync = deps.existsSync ?? fs.existsSync;
  const readFileSyncImpl = deps.readFileSync ?? fs.readFileSync;
  const rmSyncImpl = deps.rmSync ?? fs.rmSync;
...
export function cleanupLocalAdminUninstallState(
  ...
  for (const rcPath of [
    manifestRecord.manifest?.shellRcPath,
    path.join(homedirImpl(), '.zshrc'),
    path.join(homedirImpl(), '.bashrc'),
  ]) {
...
async function runAdminReset(options: AdminResetOptions): Promise<void> {
  ...
  const result = {
    ...
    local,
    nextStep:
      'Run `agentpay admin setup` to create a new wallet and re-install the managed daemon.',
  };
...
function printUninstallSummary(result: {
  ...
  if (
    result.local.oneClickInstaller.installRoot.removed &&
    result.local.oneClickInstaller.installRoot.path
  ) {
    lines.push(
      `one-click install root removed: ${result.local.oneClickInstaller.installRoot.path}`,
    );
  }
...
async function runAdminUninstall(options: AdminUninstallOptions): Promise<void> {
  ...
  const localProgress = createProgress(
    'Removing local AgentPay files and credentials',
    showProgress,
  );
